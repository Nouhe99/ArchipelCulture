...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollRectTweener : MonoBehaviour, IDragHandler
    {
        ...
        public void ScrollHorizontalByValue(float value)
        {
            int nbrElemByLine = (int)(scrollRect.viewport.rect.width / elemSize);
            float decalByWidth = nbrElemByLine * elemSize * value;
            startPos = scrollRect.content.localPosition;
            targetPos = startPos + (Vector2.right * decalByWidth);
            float duration = Vector3.Distance(startPos, targetPos) / moveSpeed;

            StopAllCoroutines();
            StartCoroutine(DoMoveRelative(duration));

            //float normalizedX = Normalize(DeNormalize(GetCurrentPos()) + (Vector2.right * decalByWidth)).x;
            //ScrollHorizontal(normalizedX);
        }
        public void ScrollVerticalOneHeight(float value)
        {
            float decalByHeight = scrollRect.viewport.rect.height * value;
            startPos = scrollRect.content.localPosition;
            targetPos = startPos + (Vector2.up * decalByHeight);
            float duration = Vector3.Distance(startPos, targetPos) / moveSpeed;
        ...
        IEnumerator DoMoveRelative(float duration)
        {
            ...
            while (currentTime < duration)
            {
                ...
                scrollRect.content.localPosition = EaseVector(currentTime, startPos, posOffset, duration);
                yield return null;
            }
