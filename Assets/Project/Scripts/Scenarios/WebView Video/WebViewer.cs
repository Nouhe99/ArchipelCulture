...
#pragma warning restore CS0162 // Code inaccessible détecté
        ...
        webViewObject.SetCenterPositionWithScale(Vector2.zero, Vector2.one);
        int widthMargin = Mathf.Max(Screen.width / 10, 64);
        int heightMargin = Mathf.Max(Screen.height / 10, 64);
        webViewObject.SetMargins(widthMargin, heightMargin, widthMargin, heightMargin); //Screen.height
        webViewObject.SetTextZoom(100);  // android only. cf. https://stackoverflow.com/questions/21647641/android-webview-set-font-size-system-default/47017410#47017410
        webViewObject.SetVisibility(true);

#if !UNITY_WEBPLAYER && !UNITY_WEBGL
        if (UrlMobile.StartsWith("http"))
        {
            webViewObject.LoadURL(UrlMobile.Replace(" ", "%20"));
        }
        else
        {
            var exts = new string[]{
            ...
            {
                var url = UrlMobile.Replace(".html", ext);
    ...
    //    {
    //        Debug.Log(webViewObject.GetCookies(UrlWebsite));
    //    }
    //    x += 90;
